...
function resolveBank(detail: ApplicationDetail) {
  const primary =
    detail.catalogSchool?.bankAccounts?.find((b) => b.isPrimary) ??
  ...
  return {
    ...
    accountNumber:
      ...
      "Not provided",
    accountName:
      primary?.accountName ?? detail.school?.bankAccountName ?? "Not provided",
...
const DetailRow: React.FC<{
  ...
  <div className="flex items-start justify-between gap-4 py-3 border-b border-gray-100 last:border-0">
    ...
    <span
      className={`text-sm font-semibold text-right break-all ${highlight ? "text-[#881337]" : "text-gray-900"}`}
    >
      {value}
    </span>
  </div>
...
const FundingPartnerDisbursementPage: React.FC = () => {
  ...
  const [transferRefError, setTransferRefError] = useState("");
  ...
  useEffect(() => {
    ...
    apiClient
      ...
      .catch(() =>
        setLoadError(
          "Failed to load funding request. The link may be expired or invalid.",
        ),
      )
  ...
  const handleDisbursed = async () => {
    if (!transferRef.trim()) {
      setTransferRefError(
        "Please enter your bank transfer reference before confirming.",
      );
      return;
    }
    if (!id) return;
    ...
    try {
      await apiClient.post(
        ...
        {
          action: "disbursed",
          note: `Disbursement confirmed by funding partner. Transfer reference: ${transferRef.trim()}`,
        },
      );
      ...
      setActionError(
        axErr.response?.data?.message ??
          "Disbursement confirmation failed. Please try again.",
      );
  ...
  const handleRejectSubmit = async () => {
    ...
    try {
      ...
      setActionError(
        axErr.response?.data?.message ??
          "Failed to record rejection. Please try again.",
      );
      setActionState("error");
    }
  };
    ...
    >
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 animate-fade-in-up">
        ...
        <p className="text-sm text-gray-500 mb-4">
          Please provide a clear reason. This will be recorded and the parent
          will be notified.
        </p>
        <textarea
          value={rejectReason}
  ...
  const amountToDisburse = Number(
    detail.amountApproved ?? detail.amountRequested,
  );
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#F8FAFC] px-4">
        <div className="bg-white rounded-3xl shadow-md border border-green-100 px-8 py-10 text-center max-w-sm w-full">
          ...
          <p className="text-sm text-gray-500 leading-relaxed">
            Thank you. We've recorded the disbursement of{" "}
            <strong>{fmt(amountToDisburse)}</strong> for application{" "}
            <strong>{detail.referenceNumber ?? detail.id}</strong>. The parent
            has been notified.
          </p>
        </div>
      </div>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#F8FAFC] px-4">
        <div className="bg-white rounded-3xl shadow-md border border-red-100 px-8 py-10 text-center max-w-sm w-full">
          ...
          <p className="text-sm text-gray-500 leading-relaxed">
            The rejection has been recorded. The SkulCredit operations team and
            the parent have been notified.
          </p>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#F8FAFC] px-4">
        <div className="bg-white rounded-3xl shadow-md border border-green-100 px-8 py-10 text-center max-w-sm w-full">
          ...
          <p className="text-xs text-gray-400 mt-1">
            Reference: {detail.referenceNumber ?? detail.id}
          </p>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#F8FAFC] px-4">
        <div className="bg-white rounded-3xl shadow-md border border-red-100 px-8 py-10 text-center max-w-sm w-full">
          <p className="text-sm font-bold text-red-700">
            This disbursement has already been rejected.
          </p>
          <p className="text-xs text-gray-400 mt-1">
            Reference: {detail.referenceNumber ?? detail.id}
          </p>
    ...
    <div className="min-h-screen bg-[#F8FAFC] py-10 px-4 md:px-8">
      ...
      <div className="max-w-2xl mx-auto space-y-6">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              ...
              <span className="font-mono font-bold">
                {detail.referenceNumber ?? detail.id}
              </span>
            </p>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-6 py-2">
          ...
          <DetailRow
            label="Admission No."
            value={detail.student?.studentId ?? "N/A"}
          />
          <DetailRow
            label="Grade / Level"
            value={detail.student?.gradeLevel ?? "N/A"}
          />
          <DetailRow
            label="School"
            value={detail.catalogSchool?.name ?? "N/A"}
          />
          <DetailRow
            label="School Tier"
            value={
            ...
            value={
              detail.serviceFeePaid
                ? detail.serviceChargeAmount
                  ? fmt(detail.serviceChargeAmount)
                  : "Paid"
                : "Not paid"
            }
          />
          <DetailRow
            ...
            value={fmt(amountToDisburse)}
            highlight
          />
        </div>
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-6 py-2">
          <div className="flex items-center justify-between py-3 border-b border-gray-50">
            <p className="text-xs font-bold text-[#881337] uppercase tracking-wide">
              School Bank Account
            </p>
            {bank.isVerified && (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50">
                  <th className="px-6 py-2.5 text-left text-xs text-gray-400 font-semibold uppercase">
                    #
                  </th>
                  <th className="px-6 py-2.5 text-left text-xs text-gray-400 font-semibold uppercase">
                    Due Date
                  </th>
                  <th className="px-6 py-2.5 text-right text-xs text-gray-400 font-semibold uppercase">
                    Amount
                  </th>
                </tr>
              </thead>
              <tbody>
                {(detail.schedule ?? []).map((s) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-3 text-right font-bold text-[#881337]">
                      {fmt(s.totalAmount)}
                    </td>
                  </tr>
                ))}
        ...
        {!alreadyProcessed && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-6 py-6 space-y-4">
            <p className="text-sm font-bold text-gray-800">
              Transfer{" "}
              <span className="text-[#881337]">{fmt(amountToDisburse)}</span> to
              the school account above, then enter your transfer reference and
              confirm below.
            </p>
            <div>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (transferRefError) setTransferRefError("");
                }}
                placeholder="e.g. NIP/240901/0012345678"
                className="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm text-gray-800 outline-none focus:border-green-500 focus:ring-2 focus:ring-green-100 transition-colors"
              />
              {transferRefError && (
                <p className="mt-1 text-xs text-red-600 font-medium">
                  {transferRefError}
              ...
              <p className="mt-1 text-xs text-gray-400">
                This reference will be recorded against the disbursement for
                audit purposes.
              </p>
            </div>
            ...
            <p className="text-xs text-gray-400 text-center">
              Once confirmed, this action cannot be undone. The parent will be
