...
const ErrorIcon: React.FC = () => (
  <svg width="13" height="13" viewBox="0 0 13 13" fill="none" aria-hidden="true" className="flex-shrink-0 mt-px">
    <circle cx="6.5" cy="6.5" r="6" stroke="#AA1701" strokeWidth="1.3" />
    <path d="M6.5 3.5V7" stroke="#AA1701" strokeWidth="1.3" strokeLinecap="round" />
    <circle cx="6.5" cy="9.2" r="0.65" fill="#AA1701" />
  </svg>
);

const FieldError: React.FC<{ msg?: string; children?: React.ReactNode }> = ({ msg, children }) =>
  msg ? (
    <p role="alert" className="flex items-center gap-1 text-xs mt-1" style={{ color: '#AA1701' }}>
      <ErrorIcon />
      {children ?? msg}
    </p>
  ) : null;
...
const RuleIcon: React.FC<{ empty: boolean; passed: boolean }> = ({ empty, passed }) => {
  if (empty)
    return (
  ...
  return (
    <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
      ...
      <path d="M4.5 4.5L8.5 8.5M8.5 4.5L4.5 8.5" stroke="white" strokeWidth="1.5" strokeLinecap="round" />
    </svg>
  );
};
...
const ParentAuthPage: React.FC = () => {
  ...
  const [checking, setChecking] = useState<CheckingState>({ email: false, phone: false });
        ...
        if (available === false) {
          setRegErrors((p) => ({
            ...
            [field === 'email' ? 'email' : 'phoneNumber']:
              field === 'email' ? '__email_taken__' : 'This phone number is already in use.',
          }));
        }
      } catch {
        /* silent */
  ...
  const handleRegChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (regErrors[name as keyof RegErrors]) setRegErrors((p) => ({ ...p, [name]: undefined }));
  ...
  const validateReg = (): boolean => {
    ...
    else if (reg.firstName.trim().length < 2) errs.firstName = 'First name must be at least 2 characters.';

    if (!reg.lastName.trim()) errs.lastName = 'Last name is required.';
    else if (reg.lastName.trim().length < 2) errs.lastName = 'Last name must be at least 2 characters.';

    if (!reg.email.trim()) errs.email = 'Email address is required.';
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(reg.email)) errs.email = 'Enter a valid email address.';
    else if (regErrors.email === '__email_taken__') errs.email = '__email_taken__';

    if (!reg.phoneNumber.trim()) errs.phoneNumber = 'Phone number is required.';
    else if (reg.phoneNumber.trim().length < 10) errs.phoneNumber = 'Phone number must be at least 10 digits.';
    else if (regErrors.phoneNumber === 'This phone number is already in use.') errs.phoneNumber = regErrors.phoneNumber;

    if (!reg.password) errs.password = 'Password is required.';
    else if (!PASSWORD_RULES.every((r) => r.test(reg.password))) errs.password = 'Password does not meet all requirements.';
    ...
    if (!reg.agreeToTerms) errs.agreeToTerms = 'You must agree to the terms to continue.';
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const axErr = err as AxiosError<{ errors?: { message: string }[]; message?: string }>;
      const msg =
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      await apiClient.post('/auth/verify-otp', { email: registeredEmail, otp: otpValue });
      setView('login');
      setLoginEmail(registeredEmail);
      setLoginError('');
    } catch (err) {
      const axErr = err as AxiosError<{ message?: string }>;
      setOtpError(axErr.response?.data?.message ?? 'Invalid or expired code. Please try again.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 w-full h-full pointer-events-none -z-10 overflow-hidden">
        ...
        <div className="absolute top-[20%] right-[15%] w-[400px] h-[400px] bg-indigo-200/40 rounded-full mix-blend-multiply filter blur-[80px] animate-blob" style={{ animationDelay: '2s' }} />
        <div className="absolute bottom-[10%] left-[30%] w-[500px] h-[500px] bg-pink-200/40 rounded-full mix-blend-multiply filter blur-[100px] animate-blob" style={{ animationDelay: '4s' }} />
      </div>

      <div className="w-full max-w-[520px] mx-auto my-10 bg-white/80 backdrop-blur-xl rounded-[2.5rem] border border-white/60 p-8 md:p-10 shadow-[0_30px_60px_-15px_rgba(136,19,55,0.15),inset_0_0_0_1px_rgba(255,255,255,0.9)] relative z-10">

        {/* ══ LOGIN ══ */}
        {view === 'login' && (
          <div className="animate-fade-in-up">
            <div className="text-center mb-8">
              <h1 className="text-3xl font-extrabold mb-3 text-transparent bg-clip-text bg-gradient-to-r from-brand via-brand-light to-brand">SkulCredit</h1>
            ...
            <form className="space-y-5" onSubmit={handleLogin}>
              {loginError && (
                <div role="alert" className="p-3 bg-red-50 text-red-600 text-sm rounded-xl font-medium border border-red-100">
                  {loginError}
                </div>
              )}
              <div className="group">
              ...
              <div className="group">
                ...
                <div className="relative">
                  ...
                  <button type="button" onClick={() => setShowLoginPw(!showLoginPw)}
                    ...
                    <Icon name={showLoginPw ? 'eye' : 'eye-off'} className="w-5 h-5" />
                  </button>
                </div>
              </div>
              ...
              <button type="submit" disabled={isLoading}
                ...
                {!isLoading && <Icon name="arrow-right" className="w-4 h-4" />}
              </button>
              <p className="text-center text-sm text-slate-500 font-medium">
                Don't have an account?{' '}
                <button type="button" onClick={() => setView('signup')} className="font-bold text-brand hover:text-brand-light transition-colors">Sign up</button>
              </p>
            </form>
          </div>
        )}
        ...
        {view === 'signup' && (
          <div className="animate-fade-in-up">
            <div className="text-center mb-6">
              <h1 className="text-2xl font-extrabold mb-1.5 text-transparent bg-clip-text bg-gradient-to-r from-brand via-brand-light to-brand">SkulCredit</h1>
              <h2 className="text-xl font-bold text-slate-900 mb-1">Create Parent Account</h2>
              <p className="text-sm text-slate-500 font-medium">Start your journey to affordable education</p>
            </div>
            {regErrors.form && (
              <div role="alert" className="mb-4 p-3 bg-red-50 text-red-600 text-sm rounded-xl font-medium border border-red-100">
                {regErrors.form}
              </div>
            )}
            <button type="button" className="flex w-full items-center justify-center gap-3 rounded-2xl border border-slate-200 bg-white py-3 text-sm font-medium text-slate-700 shadow-sm hover:bg-slate-50 transition mb-5">
              <GoogleIcon />
              Sign up with Google
            </button>
            <div className="relative mb-5 flex items-center">
              <div className="flex-grow border-t border-slate-200" />
              <span className="mx-4 flex-shrink text-xs text-slate-400 font-medium">Or sign up with email</span>
              <div className="flex-grow border-t border-slate-200" />
            </div>
            <form className="flex flex-col gap-4" onSubmit={handleRegisterSubmit} noValidate>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="text" name="firstName" placeholder="John" autoComplete="given-name"
                    value={reg.firstName} onChange={handleRegChange} className={inputCls(!!regErrors.firstName)} />
                ...
                <div>
                  ...
                  <input type="text" name="lastName" placeholder="Doe" autoComplete="family-name"
                    value={reg.lastName} onChange={handleRegChange} className={inputCls(!!regErrors.lastName)} />
              ...
              <div>
                <div className="flex items-center gap-2 mb-1.5">
                  ...
                  {checking.email && <span className="h-3 w-3 animate-spin rounded-full border-2 border-slate-400 border-t-transparent" />}
                </div>
                <input type="email" name="email" placeholder="you@example.com" autoComplete="email"
                  value={reg.email} onChange={handleRegChange} className={inputCls(!!regErrors.email)} />
                {regErrors.email && regErrors.email !== '__email_taken__' && <FieldError msg={regErrors.email} />}
                {regErrors.email === '__email_taken__' && (
                  <p role="alert" className="flex items-center gap-1 text-xs mt-1" style={{ color: '#AA1701' }}>
                    <ErrorIcon />
                    An account with this email already exists.{' '}
                    <button type="button" onClick={() => setView('login')} className="font-semibold underline ml-0.5" style={{ color: '#1D4ED8' }}>
                      Log in instead
                    </button>
                  </p>
                )}
              ...
              <div>
                <div className="flex items-center gap-2 mb-1.5">
                  ...
                  {checking.phone && <span className="h-3 w-3 animate-spin rounded-full border-2 border-slate-400 border-t-transparent" />}
                </div>
                <input type="tel" name="phoneNumber" placeholder="08011111111" autoComplete="tel"
                  value={reg.phoneNumber} onChange={handleRegChange} className={inputCls(!!regErrors.phoneNumber)} />
                <FieldError msg={regErrors.phoneNumber} />
              </div>
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-1.5">Password*</label>
                <div className="relative">
                  <input type={showRegPw ? 'text' : 'password'} name="password" placeholder="Secret@123" autoComplete="new-password"
                    value={reg.password} onChange={handleRegChange} className={inputCls(!!regErrors.password) + ' pr-12'} />
                  <button type="button" onClick={() => setShowRegPw(!showRegPw)}
                    className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-brand transition-colors">
                    <Icon name={showRegPw ? 'eye' : 'eye-off'} className="w-5 h-5" />
                  </button>
                </div>
                  ...
                  {PASSWORD_RULES.map((rule) => {
                    ...
                    return (
                      <div key={rule.label} className="flex items-center gap-1.5">
                        <span className="flex-shrink-0"><RuleIcon empty={empty} passed={passed} /></span>
                        <span className={`text-xs ${empty ? 'text-slate-400' : passed ? 'text-green-600' : 'text-[#AA1701]'}`}>{rule.label}</span>
                      </div>
                    );
                  })}
                </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input type={showConfPw ? 'text' : 'password'} name="confirmPassword" placeholder="Re-enter password" autoComplete="new-password"
                    value={reg.confirmPassword} onChange={handleRegChange} className={inputCls(!!regErrors.confirmPassword) + ' pr-12'} />
                  <button type="button" onClick={() => setShowConfPw(!showConfPw)}
                    className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-brand transition-colors">
                    <Icon name={showConfPw ? 'eye' : 'eye-off'} className="w-5 h-5" />
                  </button>
                </div>
              ...
              <div>
                <div className="flex items-center gap-3">
                  <Toggle id="agreeToTerms" checked={reg.agreeToTerms}
                    onChange={(val) => {
                      setReg((p) => ({ ...p, agreeToTerms: val }));
                      if (regErrors.agreeToTerms) setRegErrors((p) => ({ ...p, agreeToTerms: undefined }));
                    }} />
                  <label htmlFor="agreeToTerms" className="cursor-pointer text-sm text-slate-600 select-none">
                    I agree to the{' '}
              ...
              <button type="submit" disabled={isLoading || !canSubmitReg}
                className={[
              ...
              <p className="text-center text-sm text-slate-500 font-medium">
                ...
                <button type="button" onClick={() => setView('login')} className="font-bold text-brand hover:text-brand-light transition-colors">Login</button>
              </p>
            </form>
          </div>
        )}
          ...
          <div className="animate-fade-in-up py-4">
            <div className="text-center mb-8">
              ...
              <p className="text-sm text-slate-500 font-medium leading-relaxed max-w-[85%] mx-auto">
                We sent a 6-digit code to <span className="font-bold text-slate-800">{registeredEmail}</span>. Enter it below to activate your account.
              </p>
            </div>
            <form className="space-y-5" onSubmit={handleVerifyOtp}>
              {otpError && (
                <div role="alert" className="p-3 bg-red-50 text-red-600 text-sm rounded-xl font-medium border border-red-100">{otpError}</div>
              )}
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2 text-center">6-digit Verification Code</label>
                <input type="text" inputMode="numeric" placeholder="• • • • • •" required maxLength={6}
                  value={otpValue}
                  onChange={(e) => {
              ...
              <p className="text-center text-sm text-slate-500">
                ...
                {resendCooldown > 0 ? (
                  ...
                  <button type="button" onClick={handleResendOtp} className="font-bold text-brand hover:underline transition-colors">Resend code</button>
                )}
              </p>
          ...
          <div className="animate-fade-in-up py-6">
            ...
            <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); setView('login'); }}>
              ...
              <p className="text-center text-sm text-slate-500 font-medium">
                ...
                <button type="button" onClick={() => setView('login')} className="font-bold text-brand hover:text-brand-light transition-colors">Sign in</button>
              </p>
            </form>
          </div>
        )}
