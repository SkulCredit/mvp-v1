...
import { EXCHANGE, NOTIFICATION_ROUTING_KEYS, AnyNotificationPayload } from './notification.types';
import logger from '../config/logger';

function publish(payload: AnyNotificationPayload): void {
  if (!isRabbitReady()) {
    logger.warn(`RabbitMQ unavailable — notification not published: ${payload.type}`);
    return;
  }
...
export const NotificationPublisher = {
  ...
  applicationRejected(userId: string, applicationId: string, reason?: string) {
    publish({
      ...
      message:       reason ?? 'Your loan application could not be approved at this time.',
      referenceId:   applicationId,
      referenceType: 'loan_application',
    });
  },

  applicationInfoRequested(userId: string, applicationId: string, note?: string) {
    publish({
      userId,
  ...
  disbursementCompleted(userId: string, disbursementId: string, amount: number) {
    publish({
      userId,
  ...
  schoolRejected(userId: string, schoolId: string, reason?: string) {
    publish({
      ...
      message:       reason ?? 'Your school application could not be approved at this time.',
      referenceId:   schoolId,
      referenceType: 'school',
    });
  },
  ...
  accountAction(userId: string, message: string) {
    publish({ userId, type: 'account_action', title: 'Account Update', message });
  },
