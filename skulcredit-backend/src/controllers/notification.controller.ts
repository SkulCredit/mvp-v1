...
class NotificationController {
  ...
  async deleteNotification(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const deleted = await notificationService.deleteNotification(req.user!.userId, String(req.params.id));
  ...
  async registerDeviceToken(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (!token || !platform) throw new ApiError(400, 'token and platform are required');
      const record = await notificationService.registerDeviceToken(req.user!.userId, token, platform);
  ...
  async removeDeviceToken(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const removed = await notificationService.removeDeviceToken(req.user!.userId, token);
